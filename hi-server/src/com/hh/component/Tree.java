    ...
    public List<Map> sortList(List<Map> lstData, final String colCompare) {
        Collections.sort(lstData, new Comparator<Map>() {
            ...
            public int compare(Map user1, Map user2) {
                if(Integer.parseInt(user1.get(colCompare).toString()) > Integer.parseInt(user2.get(colCompare).toString())) return 1;
                else if(Integer.parseInt(user1.get(colCompare).toString()) > Integer.parseInt(user2.get(colCompare).toString())) return 0;
                else return -1;
            }
        });
        return lstData;
